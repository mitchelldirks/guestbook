...
$conn = new mysqli("localhost","root","","guestbook");
   ...
   $check="SELECT * FROM valid v INNER JOIN booking b on v.k_booking=b.booking WHERE v.k_booking='$book'";
    ...
    echo "<script>alert('$nama sudah login')</script>";
} else {   

$post="INSERT INTO valid (k_validasi,k_booking,tgl_book) VALUES ('$kval','$book','$tgl_book')";
...
if ($conn->query($att) === TRUE) {
    echo "<script>alert('Peserta sudah divalidasi, $name ')</script>";
    echo "<a class='btn btn--stripe btn--large' href='reader/index.html'>Back</a>";
...
if ($conn->query($post) === TRUE) {
    ...
    echo "<script>alert('VALIDASI TAMU GAGAL')</script>";
}}
?>